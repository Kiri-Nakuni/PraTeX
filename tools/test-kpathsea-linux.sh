...
# Official TeX Live 2026 libkpathsea gate for PraTeX.
#
# This runner never downloads or builds Kpathsea. Point it at an external
# prefix made from the official source and at a TeX tree containing the
# official cm-tfm and uptex-fonts assets described in docs/kpathsea-port-notes.md.
...
cargo_target=${PRATEX_KPATHSEA_CARGO_TARGET:-"${TMPDIR:-/tmp}/pratex-kpathsea-cargo-target"}
...
gate_root=$(mktemp -d "${TMPDIR:-/tmp}/pratex-kpathsea-linux.XXXXXXXX")
cleanup() {
...
env \
    ...
    'file_search::in_process::tests::公式libkpathseaがpratex名でtexとtfmとjfmとvfのhit_missを分ける' \
    -- --ignored --exact
...
printf 'Kpathsea %s linked gate passed: child processes 0; DVI SHA-256 %s\n' \
