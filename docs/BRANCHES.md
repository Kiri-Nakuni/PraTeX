...
| `claude1/perf-integration` | 未統合、push なし | `codex3/roadmap-integration`から分岐。fmtの行分割とtrie検証のhash、lig/kern表、列の容量、`panic = "abort"`を直した。299頁paired wall比は1.615624から**1.4689**（幾何平均1.4429）、命令数−15.99%。DVIはbyte一致、release 953 passed / 0 failed / 11 ignored。**公式TRIPは未実施**で、1.3未達 |
