...
| 部分 | `\kanjiskip`、`\xkanjiskip`と自動間隔制御 | INITEX既定0の通常glue parameterに加え、`\autospacing` / `\noautospacing`、`\autoxspacing` / `\noautoxspacing`、`\xspcode`、Unicode scalarの`\inhibitxspcode`をtyped eqtbへ持つ。`\inhibitglue` / `\disinhibitglue`はhlist-localなone-shot状態で、接続済みmain-loopの実在JFM pairだけを次の実nodeまで抑止し、node-less command、fmt、unhcopyの意味を保持する。直結和和Kは寸法・改行・DVIに効くが`\showbox` / `\lastskip` / `\lastnodetype` / `\unskip`から隠れる仮想node、Xはmaterial nodeである。確認済みunshifted hbox edgeはmaterial K/Xへ接続。discは左を遮断し、no-break/post-break末尾から右glyphへのK/Xを枝別material nodeとして保持する。list先頭・末尾class 0 JFM、shifted/vboxは未接続 |
