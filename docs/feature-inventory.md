...
| 部分 | group/if拡張表示 | `\showifs`は開いている条件を内側から外側へlevel、`\unless`、`\else`、開始行つきで表示し、空条件、`\ifcase`枝、状態不変、全mode、101回、alias、fmt、JFM境界を試験済み。`\showgroups`は未実装 |
...
[showifs試験](../tests/etex_showifs.rs)、
...
| 部分 | `\kanjiskip`、`\xkanjiskip`と自動間隔制御 | INITEX既定0の通常glue parameterに加え、`\autospacing` / `\noautospacing`、`\autoxspacing` / `\noautoxspacing`、`\xspcode`、Unicode scalarの`\inhibitxspcode`をtyped eqtbへ持つ。`\inhibitglue` / `\disinhibitglue`はhlist-localなone-shot状態で、接続済みmain-loopの実在JFM pairだけを次の実nodeまで抑止し、node-less command、fmt、unhcopyの意味を保持する。横組のdirect hbox/段落でlist両端class 0 JFMを引き、hbox / `\noindent`の先頭glue除去、hbox末尾glue零化、字下げ段落先頭の`\inhibitglue`抑止を扱う。直結和和Kは寸法・改行・DVIに効くが`\showbox` / `\lastskip` / `\lastnodetype` / `\unskip`から隠れる仮想node、Xはmaterial nodeである。確認済みunshifted hbox edgeはmaterial K/Xへ接続。discは左を遮断し、no-break/post-break末尾から右glyphへのK/Xを枝別material nodeとして保持する。class 0のindirect box/disc/unbox再評価、shifted/vbox、縦組は未接続 |
...
| 部分 | e-TeX完全性gate | `\showgroups`、未接続tracing、`\lastlinefit`、TeX--XeTのparagraph・display・math mode内方向・disc/alignment/unbox等が残る。`\showifs`、vertical discard list、四方向primitiveの明示restricted hbox限定sliceは実接続済み |
