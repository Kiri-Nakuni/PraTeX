...
| | 分岐点 `36af0ee` | 本枝 |
|---|---|---|
| 命令数 | 8,739,185,718 | **7,041,524,430**（−19.43%、`dist` profile） |
| `latex.fmt` | 21,532,699 byte / 3,949,254 行 | **13,112,867 byte / 1,317,523 行**（行数 −66.6%） |
| **paired wall 比 中央値** | **1.615624**（記録値） | **1.3578** |
| paired wall 比 幾何平均 | 1.590993（記録値） | **1.3508** |
| 命令数比（対 upLaTeX 4,412M） | 1.981 | **1.610** |

**この wall 比は機械が重い時間帯に測った。** 対の比なので drift は打ち消されるが、
絶対値は PraTeX 1.8631 s / upLaTeX 1.3667 s と、静かな時間帯の 1.18 s / 0.81 s より
大きい。比そのものも条件で動き得るので、静かな時間帯に取り直す必要がある。
命令数比 1.610 の方が条件に依らない。
