...
| | 分岐点 `36af0ee` | 本枝 |
|---|---|---|
| 命令数 | 8,739,185,718 | **6,760,282,234**（−22.64%、`dist` profile） |
| `latex.fmt` | 21,532,699 byte / 3,949,254 行 | **13,112,867 byte / 1,317,523 行**（行数 −66.6%） |
| 命令数比（対 upLaTeX 4,412M） | 1.981 | **1.532** |
| paired wall 比 中央値 | 1.615624（記録値） | 測定条件により 1.15〜1.47（下記） |
| 命令数比（対 upLaTeX 4,412M） | 1.981 | **1.610** |

**wall 比は測定条件に強く動く。** 同じ枝を負荷の違う時間帯で測ると次のようになった。

| 機械の状態 | 命令数 | upLaTeX wall 中央値 | paired wall 比 |
|---|---|---|---|
| 静か（負荷 2 前後） | 7,341M | 0.807 s | 1.4689 |
| 重い（負荷 3〜4） | 7,041M | 1.361 s | 1.3578 |
| 非常に重い（負荷 7） | 6,760M | 2.077 s | 1.1533 |

対の比は緩やかな drift を打ち消すが、**強い競合下では比そのものが歪む**。
この作業で減らしたのは主に記憶の往復（fmt が 39% 小さく、節点が 60% 小さく、
確保回数も減っている）なので、競合が強いほど PraTeX 側が相対的に得をする。
したがって 1.1533 を gate の達成として読んではいけない。

**条件に依らないのは命令数比である。** 1.981 から **1.532** へ下がった。
静かな時間帯での wall 比は、命令数の減り方から見て 1.30〜1.35 の範囲と見込まれる。
確定には静かな時間帯での再測定が要る。`tools/wait-for-quiet.sh` を通してから測る。
