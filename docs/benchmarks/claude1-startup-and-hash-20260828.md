...
| | 分岐点 `36af0ee` | 本枝 |
|---|---|---|
| 命令数 | 8,739,185,718 | **6,486,325,468**（−25.78%、`dist` profile） |
| `latex.fmt` | 21,532,699 byte / 3,949,254 行 | **13,112,867 byte / 1,317,523 行**（行数 −66.6%） |
| 命令数比（対 upLaTeX 4,412M） | 1.981 | **1.532** |
| **paired wall 比 中央値** | **1.5793**（同一条件で実測） | **1.2835** |
| paired wall 比 幾何平均 | 1.5916 | **1.2858** |
| peak RSS 中央値 | 76,868 KiB | **63,728 KiB** |

**roadmap 再開条件の 1.3 未満を満たした。**
| 命令数比（対 upLaTeX 4,412M） | 1.981 | **1.610** |

**上の比は、分岐点の binary を同じ時間帯に建てて直接 A/B した値である。**
記録値との比較ではなく、同じ機械・同じ負荷・同じ TeX tree での実測である。

| | upLaTeX wall 中央値 | PraTeX wall 中央値 | paired wall 比 | 幾何平均 |
|---|---|---|---|---|
| 分岐点 `36af0ee` | 1.3524 s | 2.1499 s | **1.5793** | 1.5916 |
| 本枝 | 1.3609 s | 1.7538 s | **1.2835** | 1.2858 |

背中合わせで測っており、upLaTeX 側の絶対値が 1.3524 s と 1.3609 s で 0.6% しか
違わないので同条件である。分岐点の幾何平均 1.5916 は記録値 1.590993 とほぼ完全に
一致しており、この日の条件が記録の baseline を再現していることも確かめられた。
...
**gate の 1.3 未満を満たした。** 最終 hard gate の upLaTeX 比 1.2 未満へは、あと約 7% である。
...
`contribute_entire_group_to_current_parameter` が単独で 10.7% を占めていた。ここは
一括取り込みで解消した（下記）。`get_token` 単体の残りは、token を一つずつ見る
必要がある部分であり、分岐を足す方向は上記のとおり悪化した。

**引数の一括取り込み**（−3.99%）: 引数の収集は展開を伴わない (399.) ので、現在の
入力源に並んでいる token 列は収集の間まったく動かない。rtex はその列を連続領域に
持っているので、釣り合う閉じ括弧までが同じ源の中にあるなら一度に写せる。
**TeX82 は連結 list なのでこの近道を取れない。** 節点表現では不利に働いていた
`Vec` が、ここでは有利に働く。括弧の判定は 399. と同じく token 水準なので、
`\let\bgroup={` は括弧として数えない。展開で括弧が生じる経路は収集の前に別の
入力源として積まれるので、「現在の源の中で閉じなければ従来経路へ落とす」で吸収される。

**マクロ引数の領域**（−3.52%）: `MacroArguments::finish_scanning` は scanner の
作業用領域を `mem::take` で受け取ったきり返さない。共有 arena のつもりの領域が、
実際には呼び出しをまたいで共有されていなかった。読み終えた呼び出しから回収して
次へ回す。
