...
**呼び出しのたびに 512 個の `Value` を作っている**（`count` 256 + `dimen` 256）。
測ると 2 µs 前後——**一回 6 µs の三分の一**である。

直すには、**触った添字だけを見る**か、**値を作り直さず使い回す**必要がある。
どちらも Vaak の側の界面を変えることになるので、**要ると分かってからにする。**
