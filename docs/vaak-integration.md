...
| **`\bump`** | **120 ns** | **860 ns** |
| `\def\z{42}` を展開するだけ | 120 ns | 120 ns |
| `\number\count1` | — | 290 ns |
...
`\bump` の 860 ns のうち、**TeX は 120 ns、Vaak が 740 ns。**

原因は `Value` が **72 バイト**あることである。`Eval` と `Esc` は 104 バイト。
**加算一回が 92 ns** かかっている——足すたびに 500 バイト近く写している。

`Value` を 16 バイトまで落とせば命令あたり数 ns になる。**そこが次。**
