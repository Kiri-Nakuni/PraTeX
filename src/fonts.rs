...
///
/// NOTE: The key is the right-hand character, so there are at most 256 entries and
/// real programs hold only a handful. TeX82 walks the lig/kern instructions linearly
/// (see 573.), and so do we: hashing a single byte costs more instructions than
/// scanning the whole program, and the entries stay in one cache line. This is looked
/// up once per character pair, so it is on the hottest path of the main loop.
