...
impl LineBreaker {
    ...
    /// doubling, for every line of every paragraph, and a `Node` is 80 bytes wide.
    /// The break points are already known at this stage, so the distance to the next
    /// one gives the count directly. The slack covers the left and right skips and
    /// the nodes transplanted out of a discretionary.
