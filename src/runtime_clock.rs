...
/// 一回のTeX runが共有する暦時刻。
///
/// TeXの整数parameterは利用者が代入できるため、`\pdfcreationdate`が途中の代入に
/// 引きずられないよう、取得時点の値とUTC offsetを別に保持する。DVI commentとfmt識別日は
/// TeX互換のためparameterを読み、snapshotはfmtへdumpせずrun開始時だけEqtbへ設定する。
